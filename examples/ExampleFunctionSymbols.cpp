...
void ExampleFunctionSymbols(const raw_pdb::RawFile& rawPdbFile, const raw_pdb::DBIStream& dbiStream)
{
    ...
    // using other hash set implementations like e.g. abseil's Swiss Tables (https://abseil.io/about/design/swisstables) is
    // *much* faster.
    ...
    // in most cases, this gives us ~90% of all function symbols already, along with their size.
    {
        ...
        for (const raw_pdb::ModuleInfoStream::Module& module : modules)
        {
            ...
            moduleSymbolStream.ForEachSymbol([&functionSymbols, &seenFunctionRVAs,
    ...
    // most of the data we need can be obtained from the module symbol streams, and the global symbol stream only offers data
    // symbols on top of that, which we are not interested in. however, there can still be public function symbols we haven't
    // seen yet in any of the modules, especially for PDBs that don't provide module-specific information.
    ...
    publicScope.Done();
    {
        ...
        for (const raw_pdb::HashRecord& hashRecord : hashRecords)
        {
            ...
            functionSymbols.push_back(FunctionSymbol{ record->S_PUB32.Name(), rva, 0u });
        }
    ...
    // this can be deduced by sorting the symbols by their RVA, and then computing the distance between the current and the next
    // symbol. this works since functions are always mapped to executable pages, so they aren't interleaved by any data symbols.
    ...
    if (symbolCount != 0u)
    {
        ...
        // note that this includes "int 3" padding after the end of a function. if you don't want that, but the actual number of
        // bytes of the function's code, your best bet is to use a disassembler instead.
        ...
        if (lastSymbol.size != 0u)
        {
            ...
            const raw_pdb::SectionContributionStream sectionContributionStream = dbiStream.CreateSectionContributionStream(
                rawPdbFile);

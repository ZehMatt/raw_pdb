...
namespace raw_pdb
{
    namespace DBI
    {
        ...
        struct StreamHeader
        {
            static constexpr uint32_t Signature = 0xFFFFFFFFu;
        ...
        struct DebugHeader
        {
            static constexpr uint16_t InvalidStreamIndex = 0xFFFFu;
            ...
            uint16_t
                sectionHeaderStreamIndex; // a dump of all section headers (IMAGE_SECTION_HEADER) from the original executable
    ...
    namespace CodeView
    {
        namespace DBI
        {
            ...
            enum class [[nodiscard]] SymbolRecordKind : uint16_t{
                S_END = 0x0006u,        // block, procedure, "with" or thunk end
                S_OBJNAME = 0x1101u,    // full path to the original compiled .obj. can point to remote locations and temporary
                                        // files, not necessarily the file that was linked into the executable
                ...
                S_COFFGROUP = 0x1137u,  // original COFF group before it was merged into executable sections by the linker, e.g.
                                        // .CRT$XCU, .rdata, .bss, .lpp_prepatch_hooks
                ...
                S_INLINESITE_END = 0x114Eu, S_PROC_ID_END = 0x114Fu, S_LPROC32_DPC = 0x1155u, S_LPROC32_DPC_ID = 0x1156u,
            ...
            enum class ProcedureFlags : uint8_t
            {
                None = 0u,
                NoFPO = 1u << 0u,
            ...
            DEFINE_BIT_OPERATORS(PublicSymbolFlags);

            // https://github.com/microsoft/microsoft-pdb/blob/master/include/cvinfo.h#L3341
            enum class CompileSymbolFlags : uint32_t
            {
                None = 0u,
            ...
            DEFINE_BIT_OPERATORS(CompileSymbolFlags);
...
#pragma pack(push, 1)
            ...
            using SYM_PUB32 = StructWithString<SYM_PUB32_BASE>;

            struct SYM_GDATA32_BASE
            {
                uint32_t typeIndex;
                uint32_t offset;
                uint16_t section;
            ...
            using SYM_GDATA32 = StructWithString<SYM_GDATA32_BASE>;
            using SYM_GTHREAD32 = SYM_GDATA32;
            using SYM_LDATA32 = SYM_GDATA32;
            using SYM_LTHREAD32 = SYM_GDATA32;

            struct SYM_OBJNAME_BASE
            {
                uint32_t signature;
                // char name[];
            };
            using SYM_OBJNAME = StructWithString<SYM_OBJNAME_BASE>;
            ...
            using SYM_SECTION = StructWithString<SYM_SECTION_BASE>;

            struct SYM_COFFGROUP_BASE
            {
                uint32_t size;
                uint32_t characteristics;
                uint32_t offset;
            ...
            using SYM_COFFGROUP = StructWithString<SYM_COFFGROUP_BASE>;

            struct SYM_THUNK32_BASE
            {
                uint32_t parent;
                uint32_t end;
                uint32_t next;
            ...
            using SYM_THUNK32 = StructWithString<SYM_THUNK32_BASE>;

            struct SYM_LPROC32_BASE
            {
                uint32_t parent;
                uint32_t end;
                uint32_t next;
            ...
            using SYM_LPROC32 = StructWithString<SYM_LPROC32_BASE>;
            using SYM_GPROC32 = SYM_LPROC32;
            using SYM_LPROC32_ID = SYM_LPROC32;
            using SYM_GPROC32_ID = SYM_LPROC32;
            using SYM_LPROC32_DPC = SYM_LPROC32;
            using SYM_LPROC32_DPC_ID = SYM_LPROC32;

            struct SYM_BLOCK32_BASE
            {
                uint32_t parent;
                uint32_t end;
                uint32_t codeSize;
            ...
            using SYM_BLOCK32 = StructWithString<SYM_BLOCK32_BASE>;

            struct SYM_LABEL32_BASE
            {
                uint32_t offset;
                uint16_t section;
                ProcedureFlags flags;
            ...
            using SYM_LABEL32 = StructWithString<SYM_LABEL32_BASE>;

            struct SYM_BUILDINFO
            {
                uint32_t typeIndex; // refers to a type index in the IPI stream
            };

            struct SYM_COMPILE3_BASE
            {
            ...
            using SYM_COMPILE3 = StructWithString<SYM_COMPILE3_BASE>;

            // https://github.com/microsoft/microsoft-pdb/blob/master/include/cvinfo.h#L3372
            struct SYM_ENVBLOCK_BASE
            {
                uint8_t flags;
                // char strings[];
            };
            using SYM_ENVBLOCK = StructWithString<SYM_ENVBLOCK_BASE>;
#pragma pack(pop)
...
#pragma pack(push, 1)
                    SYM_PUB32 S_PUB32;
                    SYM_GDATA32 S_GDATA32;
                    SYM_GTHREAD32 S_GTHREAD32;
                    SYM_LDATA32 S_LDATA32;
                    SYM_LTHREAD32 S_LTHREAD32;
                    SYM_OBJNAME S_OBJNAME;
                    SYM_TRAMPOLINE S_TRAMPOLINE;
                    SYM_SECTION S_SECTION;
                    SYM_COFFGROUP S_COFFGROUP;
                    SYM_THUNK32 S_THUNK32;
                    SYM_LPROC32 S_LPROC32;
                    SYM_GPROC32 S_GPROC32;
                    SYM_LPROC32_ID S_LPROC32_ID;
                    SYM_GPROC32_ID S_GPROC32_ID;
                    SYM_LPROC32_DPC S_LPROC32_DPC;
                    SYM_LPROC32_DPC_ID S_LPROC32_DPC_ID;
                    SYM_BLOCK32 S_BLOCK32;
                    SYM_LABEL32 S_LABEL32;
                    SYM_BUILDINFO S_BUILDINFO;
                    SYM_COMPILE3 S_COMPILE3;
                    SYM_ENVBLOCK S_ENVBLOCK;
#pragma pack(pop)
                };
            };
        } // namespace DBI
